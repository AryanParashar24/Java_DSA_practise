...
public class SearchIn2DArray {
    public static void main(String[] args) {
        int[][] arr = {            //Here we may ask why aren't we adding new int{} here as we already declaring it when we have initialised it inro this kinda
                {23, 74,1},         //arraytype with int[][] arr = ...  then we have already declared it over her ebut it isn't same for the stuff when we r asking it to
        ...
        for (int row = 0; row < arr.length; row++) {
            for (int col = 0; col < arr[row].length; col++) {
                if (arr[row][col] == target){
                    return new int[]{row, col};  //Why we added new int here for the reson look it up in the multidimentional array
                }
            }
        }
        return new int[]{-1, -1};
